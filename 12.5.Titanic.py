...
def load_data(file_name, is_train):
    ...
    if len(data.Fare[data.Fare==0]) > 0:
        ...
        for f in range(0, 3):
            ...
            # fare[f] = lstm-ptb-data['Pclass'] == f + 1]['Fare'].dropna().median()
           #找到相应的字段（仓位的等级）对应的票价，去掉空值，找到中位数进行缺失值的填充
        for f in range(0, 3):  # loop 0 to 2
            data.loc[(data.Fare==0) & (data.Pclass == f + 1), 'Fare'] = fare[f]
            #注意loc和inloc的区别

    # 年龄：使用均值代替缺失值
    # mean_age = lstm-ptb-data['Age'].dropna().mean()
    # lstm-ptb-data.loc[(lstm-ptb-data.Age.isnull()), 'Age'] = mean_age
    if is_train:
        # 年龄：使用随机森林预测年龄缺失值
    ...
    # lstm-ptb-data['Embarked'] = lstm-ptb-data['Embarked'].map({'S': 0, 'C': 1, 'Q': 2, 'U': 0}).astype(int)
    # print lstm-ptb-data['Embarked']
    ...
    # x = lstm-ptb-data[['Pclass', 'Sex', 'Age', 'SibSp', 'Parch', 'Fare', 'Embarked']]
    y = None
    if 'Survived' in data:
        y = data['Survived']
